(() => {
    ...
    (() => {
        ...
        wow_wow.init();
        if (window.matchMedia("(min-width: 991.98px)").matches) document.querySelectorAll(".menu__list > li").forEach((item => {
            item.addEventListener("mouseenter", (() => {
                const submenu = item.querySelector(".submenu");
                if (submenu) {
                    const subsubmenus = submenu.querySelectorAll(".subsubmenu");
                    let maxHeight = submenu.offsetHeight;
                    subsubmenus.forEach((subsubmenu => {
                        maxHeight = Math.max(maxHeight, subsubmenu.offsetHeight);
                    }));
                    submenu.style.minHeight = maxHeight + "px";
                    subsubmenus.forEach((subsubmenu => {
                        subsubmenu.style.minHeight = maxHeight + "px";
                    }));
                }
            }));
            item.addEventListener("mouseleave", (() => {
                ...
                if (submenu) {
                    ...
                    submenu.querySelectorAll(".subsubmenu").forEach((subsubmenu => {
                        subsubmenu.style.minHeight = "";
                    }));
                }
            }));
        }));
